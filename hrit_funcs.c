 ...
 * This file contains information on the actual image scan
 *
...
/*******************************************************************************
 * Read the PRO file associated with a SEVIRI timeslot.
 * This file contains information on the planned image scan and calibration
 *
...
/*      Seek to the beginning of the useful part of the prologue (only care about image desc and radiometric calibration*/
/*      This seek value should be constant, has not changed in 11 years of MSG data*/
...
/*******************************************************************************
 * Allocate the space in memory to be used for the SEVIRI image
 * Space is allocated for the HRV data is required, although it is unsupported.
 *
...
int alloc_imagearr(uint nbands, const uint *band_ids,struct seviri_native_data *d)
{
     ...
     length_vir=d->header.ImageDescription.ReferenceGridVIS_IR.NumberOfLines*d->header.ImageDescription.ReferenceGridVIS_IR.NumberOfColumns;
     length_hrv=d->header.ImageDescription.ReferenceGridHRV.NumberOfLines*d->header.ImageDescription.ReferenceGridHRV.NumberOfColumns;
...
/*      Put image info in the correct place*/
/*      This is done for consistency with the .nat reader. HRIT files*/
/*      contain different data, so need to move things around*/
     ...
     if (seviri_get_dimension_data(dimens, &d->marf_header, bounds, line0, line1,column0, column1, lat0, lat1, lon0, lon1)) {
...
/*      Loop over each band and each segment. Note: VIR only, no HRV*/
     for (i=0;i<n_bands;i++)     for (j=0;j<8;j++)     read_data_oneseg(bnames[i][j],j,i+1,d,dimens);
