...
Once read in, the data can then be accessed, changed (subsetted, calibrated, etc.) and/or written to a new file.  In addition, the data may be preprocessed to obtain, radiance, reflectance, and/or brightness temperature, for any set of channels, along with pixel time, latitude, longitude, solar zenith angle, solar azimuth angle, sensor viewing zenith angle, and sensor viewing azimuth angle.
...
The main code is C89 compliant and is dependent only on the C standard library.  As such, it should compile with any modern C compiler and probably with most C++ compilers.  A simple GNU Makefile exists in which the compiler and its options can be changed.  The default setup is for GCC and should work on any Linux or MacOS systems with programming tools installed and probably most modern UNIX systems.  In this case executing `make' in the source code directory will compile the code.
...
In addition to the C interface seviri_native_util has a Fortran interface that provides access to the most important functionality.  If the Fortran interface is desired uncomment the indicated line in the Makefile and adjust the Fortran compiler and options as desired.  The default setup is for GFortran.
...
Working example programs for both C and Fortran are included with the distribution 'example_c' and 'example_f90'.  These are fully commented in the code and should be enough to get started using seviri_native_util.  These programs take a single argument: a MSG2-SEVI-MSG15-* filename, read sub-images of reflectance in two bands and brightness temperature in two bands, and print the values for the central pixel.
