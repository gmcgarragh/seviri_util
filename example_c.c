...
int main(int argc, char *argv[]) {
     ...
     printf("i_column:                     %d\n", i_column);
     printf("i_pixel:                      %d\n", i_pixel);
     printf("Julian Day Number:            % e\n", preproc.time[i_pixel]);
     printf("latitude:                     % e\n", preproc.lat [i_pixel]);
     printf("longitude:                    % e\n", preproc.lon [i_pixel]);
     printf("solar zenith angle:           % e\n", preproc.sza [i_pixel]);
     printf("solar azimuth angle:          % e\n", preproc.saa [i_pixel]);
     printf("viewing zenith angle:         % e\n", preproc.vza [i_pixel]);
     printf("viewing azimuth angle:        % e\n", preproc.vaa [i_pixel]);
     printf("0.635 refectance:             % e\n", preproc.data[0][i_pixel]);
     printf("1.64  refectance:             % e\n", preproc.data[1][i_pixel]);
     printf("8.70  brightness temperature: % e\n", preproc.data[2][i_pixel]);
     printf("10.80 brightness temprature:  % e\n", preproc.data[3][i_pixel]);
